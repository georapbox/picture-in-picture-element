...
const sourceTemplate = (slot = '', classAttr = '') => escapeHTML(`<picture-in-picture${classAttr}>${slot}</picture-in-picture>`);
...
import(componentUrl).then(res => {
  ...
  const renderElement = (isCustomized = true) => {
    ...
    htmlSrcEl.innerHTML = sourceTemplate(slotTemplate, isCustomized ? ' class="custom-styles"' : '');
